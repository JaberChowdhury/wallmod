...
use gpui::*;
use gpui_component::{h_flex, spinner::Spinner, v_flex, ActiveTheme, Sizable, StyledExt};
...
impl Render for WallmodView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let loading_msg = if let crate::app::AppState::Loading(_, ref s) = self.app.state {
            s.clone()
        } else {
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            .text_sm()
                            .font_bold()
                            .child(format!("⚡ {} (Animated Loading Dot Active)", loading_msg)),
                    )
                    .into_any_element()
            } else {
