...
pub fn render_histogram(data: Option<&HistogramData>, cx: &mut Context<crate::ui::WallmodView>) -> impl IntoElement {
    v_flex()
        .w_full()
        .p_4()
        .gap_2()
        ...
        .child(
            h_flex().justify_between().items_center()
                .child(div().text_sm().font_bold().text_color(cx.theme().primary).child("Color Waveform Histogram"))
                .child(div().text_xs().text_color(cx.theme().muted_foreground).child("Logarithmic Scale"))
        )
        .child(
            if let Some(hist) = data {
                ...
                for i in 0..64 {
                    ...
                    let norm = if avg > 0.0 { (avg + 1.0).ln() / max_log } else { 0.05 };
