...
use gpui_component::{
    ...
    v_flex, h_flex, ActiveTheme, Sizable, Selectable, StyledExt,
};
                        ...
                        .child(
                            if daemon {
                                v_flex().gap_2().p_3().border_1().border_color(cx.theme().border).rounded_lg().bg(cx.theme().secondary)
                                    .child(div().text_xs().font_bold().child("Daytime Theme (06:00 - 18:00)"))
                                    .child(
                                        h_flex().gap_1().flex_wrap()
                                            .children(PRESET_NAMES.iter().take(4).map(|&name| {
                                                let is_sel = day_t == name;
                                                let n = name.to_string();
                                                Button::new(SharedString::from(format!("day_{}", name))).label(name).small().selected(is_sel).on_click(cx.listener(move |this, _, _, cx| {
                                                    this.app.day_theme = n.clone(); cx.notify();
                                                }))
                                            }))
                                    )
                                    .child(div().text_xs().font_bold().pt_1().child("Nighttime Theme (18:00 - 06:00)"))
                                    .child(
                                        h_flex().gap_1().flex_wrap()
                                            .children(PRESET_NAMES.iter().skip(4).take(4).map(|&name| {
                                                let is_sel = night_t == name;
                                                let n = name.to_string();
                                                Button::new(SharedString::from(format!("night_{}", name))).label(name).small().selected(is_sel).on_click(cx.listener(move |this, _, _, cx| {
