...
pub fn get_preset_shades(name: &str) -> Vec<[u8; 3]> {
    match name {
        ...
        "Rose Pine Moon" => vec![[235, 111, 146], [246, 193, 119], [234, 154, 151], [196, 167, 231], [156, 207, 216]],
        ...
        "Everforest Dark" => vec![[231, 138, 78], [216, 166, 87], [167, 192, 128], [127, 180, 202], [211, 134, 155]],
        "Ayu Dark" => vec![[255, 51, 51], [255, 151, 56], [255, 213, 128], [184, 204, 82], [54, 163, 217]],
        "Monokai Pro" => vec![[255, 97, 136], [252, 152, 103], [255, 216, 102], [169, 220, 118], [120, 220, 232]],
        "Night Owl" => vec![[239, 83, 80], [247, 140, 108], [255, 235, 149], [34, 218, 110], [130, 170, 255]],
...
pub fn extract_lut_shades(path: &PathBuf) -> Vec<[u8; 3]> {
    ...
    if let Ok(content) = std::fs::read_to_string(path) {
        ...
        for line in content.lines() {
            let line = line.trim();
            if line.is_empty() || line.starts_with('#') || line.starts_with('A') || line.starts_with('T') || line.starts_with('L') || line.starts_with('D') {
                continue;
            }
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() >= 3 {
                if let (Ok(r), Ok(g), Ok(b)) = (parts[0].parse::<f32>(), parts[1].parse::<f32>(), parts[2].parse::<f32>()) {
                    let r8 = if r <= 1.0 { (r * 255.0) as u8 } else { r.clamp(0.0, 255.0) as u8 };
                    let g8 = if g <= 1.0 { (g * 255.0) as u8 } else { g.clamp(0.0, 255.0) as u8 };
                    let b8 = if b <= 1.0 { (b * 255.0) as u8 } else { b.clamp(0.0, 255.0) as u8 };
